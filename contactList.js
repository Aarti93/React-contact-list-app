...
var ContactFormComponent = React.createClass({
  ...
  validate: function() {
    this.setState({errorMessage:"", emailTextBoxClass:"textBox", nameTextBoxClass:"textBox"});

    if(!(this.newContact.name && /^[a-zA-Z][a-z A-Z]*$/.test(this.newContact.name))) {    
      this.setState({errorMessage:"Please enter valid name", nameTextBoxClass:"textBoxInvalid"});
      return false;
    }

    if(!(this.newContact.email && /.+@.+\..+/.test(this.newContact.email))) {
      this.setState({errorMessage:"Please enter valid email", emailTextBoxClass:"textBoxInvalid"}); //only setState re renders component..should not do this.state.error="hello"
      return false;
    }
  ...
  onSubmitForm: function() {
    ...
    if(validation) { //Empty the fields and update the contacts
  ...
  render: function () {
    var errorDivStyle = {color:'red', textAlign: 'left', display: 'block', 'marginLeft': '6%'}; //inline css
    return (
      <div>
        <div className="mainHeading">Add a new contact</div>
        <div className="form">
          <input type="text" ref="nameValue" className={this.state.nameTextBoxClass}  placeholder="Full Name" />
          <input type="text" ref="emailValue" className={this.state.emailTextBoxClass} placeholder="Email Address" />
          <div style={errorDivStyle}>{this.state.errorMessage}</div> 
          <input type="button" defaultValue="Add Contact"  className="button" onClick={this.onSubmitForm}/>
        </div>
      </div>
      );
  }
  ...
  render: function () {
    return (
      <div className="contactDetails">
        <input type="button" className="deleteContactButton"  defaultValue="x" onClick={this.props.onDeleteClick}/>
        <br/>
        <div className="fieldName">Name :</div>
        <div className="fieldValue"> {this.props.contact.name}</div>
        <br/>
        <div className="fieldName">Email :</div>
        <div className="fieldValue"> {this.props.contact.email}</div>
...
var ContactListComponent = React.createClass({
  newContact : {
    ...
    componentDidMount: function() {
      ...
      var url = "http://localhost:1122/getJsonData";
        ...
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
           ...
           if (this.isMounted()) {
            if (!contactsResponse) {
              ...
              this.setState({contacts: contactsResponse.contacts.slice()});
            }
          }
    ...
    saveContactList: function() {
      var xmlhttp = new XMLHttpRequest();
      var url = "http://localhost:1122/saveList";
      xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
    ...
    onSubmitForm: function (newContact)   {
      ...
      this.setState({contacts: this.state.contacts.concat(addContact)}); //setstate rerenders component
    },
    ...
    contactDetail: function (contact) {
      ...
      return (<ContactDetails contact={contact} key={id} onDeleteClick={this.onDeleteClick.bind(this,id)}/>)
    },
    render: function () {
      ...
      return (
        <div>
          <div className="contacts">
            <div className="mainHeading">CONTACTS</div>
            <div>{contactDetailComponentNodes}</div>
          </div>
          <div className="addContactDiv">
            <ContactFormComponent onSubmitForm={this.onSubmitForm}/>
            <input type="button" className="button" defaultValue="Save this list" onClick={this.saveContactList} />
          </div>
        </div>
        )
